...
export default function ExerciseHistory() {
  ...
  const { exercises, categories, sessions, sets, sessionRecords, records } = useData()
  ...
  const exerciseRecords = useMemo(() => sessionRecords.filter((item) => item.exercise_id === exerciseId).sort((a, b) => new Date(a.recorded_at) - new Date(b.recorded_at)), [exerciseId, sessionRecords])
  ...
  const chartItems = exerciseRecords
  ...
  return (
    <main className="content-page history-page">
      ...
      <section className="record-hero glass-card"><span className="record-medal"><Trophy /></span><div><span className="eyebrow">Current personal record</span><h2>{record ? Number(bodyweight ? record.best_reps_at_weight : record.best_weight).toLocaleString() : '—'} <small>{bodyweight ? (exercise.unit === 'seconds' ? 'sec' : 'reps') : exercise.unit}</small></h2><p>{record ? bodyweight ? 'Best duration or repetition result.' : `Estimated 1RM ${Number(record.best_est_1rm).toFixed(1)} ${exercise.unit}` : 'Log a set to establish your baseline.'}</p></div></section>
      <section className="section-block"><div className="section-heading"><div><span className="eyebrow">Permanent best per session</span><h2>Session progress</h2></div><span className="count-pill">{exerciseRecords.length}</span></div><div className="glass-card chart-card large history-chart-card">{chartItems.length ? <MiniChart scrollable values={chartItems.map((item) => Number(bodyweight ? item.best_reps_at_weight : item.best_est_1rm))} labels={chartItems.map((item) => new Date(item.recorded_at).toLocaleDateString(undefined, { month: 'short', day: 'numeric' }))} format={(value) => bodyweight ? `${value} ${exercise.unit === 'seconds' ? 'sec' : 'reps'}` : `${Number(value).toFixed(1)} ${exercise.unit}`} /> : <div className="empty-chart">Not enough data yet</div>}</div></section>
      <section className="section-block"><div className="section-heading"><div><span className="eyebrow">Raw sets · last 3 months</span><h2>Detailed history</h2></div><span className="count-pill">{exerciseSets.length}</span></div><p className="form-note">Session bests and PRs remain permanent after detailed sets expire.</p><div className="session-history">{grouped.map((group) => <article className="glass-card history-session" key={group.session?.id}><header><span><CalendarDays />{new Date(group.session?.started_at).toLocaleDateString(undefined, { weekday: 'short', day: 'numeric', month: 'long', year: 'numeric' })}</span><small>{group.sets.length} sets</small></header>{group.sets.map((set) => <div className="history-set" key={set.id}><span>SET {String(set.set_number).padStart(2, '0')}</span><strong>{Number(set.weight).toLocaleString()} <small>{exercise.unit}</small></strong><span>×</span><strong>{set.reps} <small>{exercise.unit === 'seconds' ? 'sec' : 'reps'}</small></strong>{set.is_pr && <span className="pr-chip"><Trophy /> PR</span>}</div>)}</article>)}{!grouped.length && <div className="empty-state glass-card"><Dumbbell /><h3>No recent set details</h3><p>{exerciseRecords.length ? 'Older session bests remain in the progress graph.' : 'Add this exercise to your next workout.'}</p></div>}</div></section>
    </main>
  )
}
