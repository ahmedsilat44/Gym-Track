...
export default function Social() {
  ...
  const { user } = useAuth()
  const { profiles, friendships, feedPosts, postLikes, postComments, routines, createPost, toggleLike, addComment, deletePost, sendFriendRequest, respondToFriendRequest, removeFriend, copyRoutine } = useData()
  ...
  const publish = async (event) => {
    ...
    await perform(() => createPost({ caption }), 'Posted to your friends.')
    setCaption('')
  }
  ...
  return (
    <main className="content-page social-page">
      ...
      {tab === 'feed' && <div className="social-layout">
        ...
        <aside className="glass-card social-sidebar"><span className="eyebrow">Your circle</span><h2>{friendIds.length} friend{friendIds.length === 1 ? '' : 's'}</h2><p>Only accepted friends can see your friends-only updates and shared routines.</p><button className="secondary-button" onClick={() => setTab('friends')}><UserPlus /> Manage friends</button></aside>
      </div>}
