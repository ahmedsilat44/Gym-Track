import { Check, Dumbbell, Eye, Heart, MessageCircle, MoreHorizontal, Plus, Search, Send, Share2, Trash2, UserCheck, UserPlus, Users, X } from 'lucide-react'
...
export default function Social() {
  ...
  const discoverProfiles = useMemo(() => {
    ...
    return profiles
      .filter((profile) => profile.id !== user.id && `${profile.display_name} ${profile.username}`.toLowerCase().includes(normalizedQuery))
      .sort((a, b) => {
        const aStarts = a.display_name.toLowerCase().startsWith(normalizedQuery) || a.username.toLowerCase().startsWith(normalizedQuery)
        const bStarts = b.display_name.toLowerCase().startsWith(normalizedQuery) || b.username.toLowerCase().startsWith(normalizedQuery)
        return Number(bStarts) - Number(aStarts) || a.display_name.localeCompare(b.display_name)
      })
      .slice(0, 20)
  }, [normalizedQuery, profiles, user.id])
    ...
    <main className="content-page social-page">
      ...
      {tab === 'feed' && <div className="social-layout">
        <section className="social-main">
          <form className="glass-card post-composer" onSubmit={publish}><button type="button" className="avatar small avatar-link" onClick={() => navigate(`/profile/${user.id}`)} aria-label="Open your profile">{(currentProfile?.display_name || 'A').slice(0, 2).toUpperCase()}</button><textarea aria-label="Share a training update" rows="2" maxLength="1000" value={caption} onChange={(event) => setCaption(event.target.value)} placeholder="Share a training update…" /><div className="composer-actions"><select value={visibility} onChange={(event) => setVisibility(event.target.value)} aria-label="Post visibility"><option value="friends">Friends only</option><option value="public">Public</option></select><button className="primary-button compact" disabled={busy || !caption.trim()}><Send /> Post</button></div></form>
          {availableRoutines.length > 0 && <section className="shared-routine-shelf"><div className="section-heading"><div><span className="eyebrow">Browse plans</span><h2>Shared routines</h2></div><span className="count-pill">{availableRoutines.length}</span></div><div>{availableRoutines.map((routine) => { const author = profiles.find((profile) => profile.id === routine.user_id); const days = routineDays.filter((day) => day.routine_id === routine.id); const dayIds = days.map((day) => day.id); const exerciseCount = routineExercises.filter((exercise) => dayIds.includes(exercise.routine_day_id)).length; return <article className="glass-card community-routine" key={routine.id}><span className="routine-visibility">{routine.visibility === 'public' ? 'Public' : 'Friends'}</span><h3>{routine.name}</h3><p>{routine.description || `A routine from ${author?.display_name || 'an athlete'}.`}</p><small>{author?.display_name || 'Athlete'} · {days.length} days · {exerciseCount} exercises</small><button className="secondary-button compact" disabled={busy} onClick={() => setPreviewRoutine(routine)}><Eye /> View plan</button></article> })}</div></section>}
          <div className="feed-list">{feedPosts.map((post) => {
            ...
            return <article className="glass-card feed-post" key={post.id}>
              <header><button className="post-author" onClick={() => navigate(`/profile/${post.user_id}`)}><span className="avatar small">{(author?.display_name || 'A').slice(0, 2).toUpperCase()}</span><span><strong>{author?.display_name || 'Athlete'}</strong><small>@{author?.username || 'athlete'} · {timeAgo(post.created_at)} <i className={`post-visibility ${post.visibility}`}>{post.visibility === 'public' ? 'Public' : 'Friends'}</i></small></span></button>{post.user_id === user.id ? <button className="post-menu danger-ghost" onClick={() => window.confirm('Delete this post?') && perform(() => deletePost(post.id))} aria-label="Delete post"><Trash2 /></button> : <MoreHorizontal />}</header>
              ...
              {(openComments[post.id] || replies.length > 0) && <div className="comment-area">{replies.map((reply) => { const writer = profiles.find((item) => item.id === reply.user_id); return <div className="comment" key={reply.id}><span className="avatar tiny">{(writer?.display_name || 'A').slice(0, 1).toUpperCase()}</span><p><strong>{writer?.display_name || 'Athlete'}</strong> {reply.body}<small>{timeAgo(reply.created_at)}</small></p></div> })}<form onSubmit={(event) => submitComment(event, post.id)}><input aria-label="Write a comment" maxLength="500" value={comments[post.id] || ''} onChange={(event) => setComments({ ...comments, [post.id]: event.target.value })} placeholder="Write a comment…" /><button aria-label="Post comment" disabled={busy || !(comments[post.id] || '').trim()}><Send /></button></form></div>}
            </article>
      ...
      {tab === 'friends' && <section className="friends-panel">
        {incoming.length > 0 && <div className="friend-section"><div className="section-heading"><div><span className="eyebrow">Waiting for you</span><h2>Friend requests</h2></div><span className="count-pill">{incoming.length}</span></div><div className="friend-grid">{incoming.map((request) => { const profile = profiles.find((item) => item.id === request.requester_id); return <div className="glass-card friend-card" key={request.id}><button className="avatar avatar-link" onClick={() => navigate(`/profile/${request.requester_id}`)} aria-label={`Open ${profile?.display_name || 'athlete'} profile`}>{(profile?.display_name || 'A').slice(0, 2).toUpperCase()}</button><span><strong>{profile?.display_name}</strong><small>@{profile?.username}</small></span><div><button className="primary-button compact" disabled={busy} onClick={() => perform(() => respondToFriendRequest(request.id, true), 'Friend request accepted.')}><Check /> Accept</button><button className="icon-button" disabled={busy} onClick={() => perform(() => respondToFriendRequest(request.id, false))}><X /></button></div></div> })}</div></div>}
        <div className="friend-section"><div className="section-heading"><div><span className="eyebrow">Accepted connections</span><h2>Your friends</h2></div><span className="count-pill">{friendIds.length}</span></div><div className="friend-grid">{accepted.map((connection) => { const id = connection.requester_id === user.id ? connection.addressee_id : connection.requester_id; const profile = profiles.find((item) => item.id === id); return <div className="glass-card friend-card" key={connection.id}><button className="avatar avatar-link" onClick={() => navigate(`/profile/${id}`)} aria-label={`Open ${profile?.display_name || 'athlete'} profile`}>{(profile?.display_name || 'A').slice(0, 2).toUpperCase()}</button><span><strong>{profile?.display_name || 'Athlete'}</strong><small>@{profile?.username}</small><p>{profile?.bio}</p></span><button className="friend-status" onClick={() => window.confirm(`Remove ${profile?.display_name || 'this friend'}?`) && perform(() => removeFriend(connection.id), 'Friend removed.')}><UserCheck /> Friends</button></div> })}{!accepted.length && <div className="empty-state glass-card"><Users /><h3>No friends yet</h3><p>Find your training partners below and send a request.</p></div>}</div></div>
        <div className="friend-section"><div className="section-heading"><div><span className="eyebrow">Find your crew</span><h2>Discover athletes</h2></div></div><label className="search-box" htmlFor="athlete-search"><Search /><input id="athlete-search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search name or username" autoComplete="off" aria-describedby="athlete-search-help" /></label><p id="athlete-search-help" className="search-help">Enter at least 2 characters. Athlete directory stays hidden until you search.</p>{normalizedQuery.length >= 2 && <div className="friend-grid discover-grid">{discoverProfiles.map((profile) => { const connection = relationship(profile.id); const isFriend = connection?.status === 'accepted'; const pendingIn = connection?.status === 'pending' && connection.addressee_id === user.id; return <div className="glass-card friend-card" key={profile.id}><button className="avatar avatar-link" onClick={() => navigate(`/profile/${profile.id}`)} aria-label={`Open ${profile.display_name} profile`}>{profile.display_name.slice(0, 2).toUpperCase()}</button><span><strong>{profile.display_name}</strong><small>@{profile.username}</small><p>{profile.bio}</p></span>{isFriend ? <span className="friend-status static"><UserCheck /> Friends</span> : connection ? <span className="friend-status static">{pendingIn ? 'Requested you' : 'Request sent'}</span> : <button className="secondary-button compact" disabled={busy} onClick={() => perform(() => sendFriendRequest(profile.id), 'Friend request sent.')}><Plus /> Add</button>}</div> })}{!discoverProfiles.length && <div className="empty-state glass-card discover-empty"><Search /><h3>No athletes found</h3><p>Try another name or username.</p></div>}</div>}
        {outgoing.length > 0 && <p className="form-note">You have {outgoing.length} pending friend request{outgoing.length === 1 ? '' : 's'}.</p>}</div>
      </section>}
      <RoutinePreviewModal routine={previewRoutine} routineDays={routineDays} routineExercises={routineExercises} authorName={profiles.find((profile) => profile.id === previewRoutine?.user_id)?.display_name} busy={busy} onClose={() => setPreviewRoutine(null)} onCopy={() => perform(async () => { await copyRoutine(previewRoutine.id); setPreviewRoutine(null); navigate('/planner') }, 'Routine copied to your planner.')} />
    </main>
  )
}
