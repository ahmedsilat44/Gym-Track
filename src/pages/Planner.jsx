      ...
      {form && <Modal title={form.id ? 'Edit routine' : 'Create routine'} onClose={() => setForm(null)} footer={<><button className="secondary-button" onClick={() => setForm(null)}>Cancel</button><button className="primary-button compact" form="routine-form" disabled={busy}>{busy ? 'Saving…' : 'Save routine'} <Check /></button></>}>
        <form id="routine-form" className="modal-form routine-form" onSubmit={submitRoutine}>
          ...
          {form.days.map((day, dayIndex) => <section className="routine-builder-day" key={day.id ?? dayIndex}>
            <header><span>{String(dayIndex + 1).padStart(2, '0')}</span><strong>Training day</strong>{form.days.length > 1 && <button type="button" aria-label={`Remove training day ${dayIndex + 1}`} onClick={() => setForm({ ...form, days: form.days.filter((_, index) => index !== dayIndex) })}><Trash2 /></button>}</header>
            ...
            <div className="planned-exercises">{day.exercises.map((planned, exerciseIndex) => <div className="planned-exercise" key={`${planned.exercise_id}-${exerciseIndex}`}>
              <div className="planned-exercise-title"><span className="exercise-mini"><Dumbbell /></span><label>Exercise<select value={planned.exercise_id} onChange={(event) => { const selected = activeExercises.find((item) => item.id === event.target.value); updatePlanned(dayIndex, exerciseIndex, { exercise_id: selected.id, exercise_name: selected.name, unit: selected.unit }) }}>{activeExercises.filter((item) => item.id === planned.exercise_id || !day.exercises.some((other, index) => index !== exerciseIndex && other.exercise_id === item.id)).map((item) => <option value={item.id} key={item.id}>{item.name} — {categories.find((category) => category.id === item.category_id)?.name || 'Unassigned'}</option>)}</select></label><button type="button" aria-label={`Remove ${planned.exercise_name}`} onClick={() => updateDay(dayIndex, { exercises: day.exercises.filter((_, index) => index !== exerciseIndex) })}><Trash2 /></button></div>
