...
- **Time-Series Charts:** Treat progression as compact instrumentation inside a bounded 16px-radius panel. Use sparse low-contrast horizontal grid lines, muted monospaced axes, a single 2.5px teal line with rounded joins, and only a faint teal area wash and restrained glow. Emphasize the latest date and point in teal, adapt label density to the available width, provide a complete text summary for assistive technology, and replace the plot with a plain empty state when no finite values exist.
- **Metric Rails:** When one chart can truthfully represent several measures, place a horizontally scrollable pill rail directly against the chart container. Keep every option on one line with a minimum 42px touch height; inactive options use a raised navy surface and muted text, while the selected option uses solid teal with dark text and exposes its pressed state semantically. Switching a metric updates the chart, its current-best readout, unit, scope, and accessible summary as one state change.
